...
summary(attr)
```
As can be seen from the summary, there are more males than females, but, since the class has an odd number of students, this distinction is not significant. 
Instead, ethnicity is pretty unequal, showing 11 immigrants and 14 Italians. Related to grade, the minimum is 3 despite the grade normally starts from 0, while the maximum is 10. Both the median and the mean are around 6, which is the sufficiency. 
...
These attributes could be mapped in many different ways considering the shape, the colour and the size of nodes. Below we present three versions of the network. 
...
In this representation, both gender and ethnicity are evident, while the grade is not accurately represented by the size. In this last case, a size vector has been created, so that the size of nodes is proportional to their grade, but still without being able to distinguish the precise difference in terms of grades.

In the following chunk, a legend is added to guide the user in the reading of the network:

- ethnicity, through colours, represents 1st and 2nd generation of immigrants, plus the Italians;
...
Despite ethnicity is not so evident as before, in this case, the gender is discernible through the hue of nodes, while the grade through the use of lightness in the choice of colours, despite grade is a discrete variable described by a continuous gradient. In fact, darker colours indicate higher grades and the difference is perceptible to the previous case where size was used. 

Notice that the application of size variation concerning gender or ethnicity may unconsciously suggest a social superiority of one of the categories, which does not exist. Moreover, colours and shapes are sufficient to represent categorical variables with less than 3 values, therefore size was not considered.
...
This third version of the network exploits `visNetwork` to build an interactive visualization of the class, reproducing the first version forehead proposed, where:

- shapes indicate the gender (i.e. females as circles, males as squares);
- colours indicate the ethnicity, as shown in the legend;
...
These values are high above the average of `r avg_outdegree` $\pm$ `r round(sd(people[,"Out degree"]),2)` but do reach extreme cases, such as a centralized network (i.e., maximum out-degree). Therefore, a visual interpretation of the out-degree distribution in a bar plot graph may help understand the underlying reasons for low reference points.    
...
Once again, the out-degree measure shows an imbalanced situation assessing a significant gap between the lowest extreme cases, such as 0, and the highest ones. The imbalance is determined by the three central nodes that become the primary senders of bullying acts. 
...
For completeness, some tests are run for ethnicity too. Since there are three groups of ethnicity among students, ANOVA tests are used to investigate about pairwise differences. As `TukeyHSD` shows, the p-values is not significant, therefore ethnicity is not discriminant for bullying or being bullied. 
...
A **permutation-based test** (also called re-randomization test) is a type of statistical significance test in which the distribution of the test under the null hypothesis is obtained by calculating all possible values of the test statistic under all possible rearrangements of the observed data points.

In the following chunk, this test is performed on 1,000 permutations of the grade over students' in-degree, while computing the Pearson correlation coefficient between them. As the histogram shows, after all the permutations, a Gaussian distribution is reached within the range of values of Pearson coefficient (i.e. $[-1,1]$), since it is likely to find no correlations between random permutations of grade and in-degree than perfect ones. Moreover, as indicated by the straight vertical line, the Pearson coefficient $\rho$ indicates that, after $1000$ permutations, the correlation is more statistically significant than by chance. 
...
$$
The most centralized network is the one that has a single node in the centre, which that links every other node in the network. In this case, the maximum degree is $(n-1)$, therefore the centralization index is:
...
In the studied network, the centralization index is `r sna::centralization(net,sna::degree)`, which indicates a fairly decentralized network. 

Since the bullying network is directed, it is possible to distinguish the centralization index into in degree and out degree. As the table shows, the in degree is higher than the out degree centralization index, indicating a more centralized bullied network (i.e. few kids are bullied from many) and a less centralized bullying network (i.e. nearly everybody bullies others).

Notice that both indexes are higher than the previous centralization index ($0.11$), suggesting the presence of two more centralized sub-networks. 
...
Considering the maximal (i.e. $1$) and minimal (i.e. $0$) values reachable in terms of the centralization index, the computed ones are closer to the minimal reference point than the maximal, leading to the conclusion of a decentralized network. 
...
Let’s start with a census about dyads, which will return how many mutual, asymmetric and null dyads there are in the network. 
```{r}
sna::dyad.census(net)
```
...
As further step, the reciprocity index is computed as the number of mutual dyads (i.e. $M$) in the network over the total amount of edges, i.e. sum of mutuals ($M$) and asymmetrical edges ($A$):
...
If there wasn’t any mutual edge, reciprocity would have been 0:

$$
Reciprocity_{min} = \frac{2\cdot M}{2\cdot M+A} = \frac{2\cdot 0}{2\cdot 0+A}=0
$$

If instead all edges were mutual, the highest reachable value would have been 1:

$$
Reciprocity_{max} = \frac{2\cdot M}{2 \cdot M+A} = \frac{2\cdot M}{2\cdot M}=1
$$

Concerning the edgewise or tie or arc-based reciprocity, i.e. the proportion of edges that are reciprocated, we get a reciprocity index of `r sna::grecip(net, measure="edgewise")` and thus indicating the lack of mutual edges, albeit not their absence.

Considering instead the dyadic measure for reciprocity, we obtain the proportion of mutual and null edges over the total (i.e. mutual $M$, asymmetrical $A$ and null edges $N$), getting a `r sna::grecip(net, measure="dyadic")` reciprocity score:
...
By removing null edges, the index falls to `r sna::grecip(net, measure="dyadic.nonnull")`, indicating that over the 5% of edges is mutual and therefore that over 5% of students bully each other mutually:

$$
Reciprocity_{\text{dyadic non null}} = \frac{M+N}{M+N+A} 
$$
