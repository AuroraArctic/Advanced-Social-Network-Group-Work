...
results$Value = c(people[which.max(people$indegree),]$Name,
                  max(people$indegree),
                  nodes,
                  min(people$indegree),
                  0, 
...
people$norm_outdegree = round(people$outdegree / nodes, 3)

people %>%
  arrange(desc(outdegree)) %>%
    formattable(align=c("l",rep("l",5)),
                list(
                    indegree = color_bar("lightgreen"),
                    outdegree = color_bar("lightpink"),
                    degree = color_bar("lightblue")))
```
