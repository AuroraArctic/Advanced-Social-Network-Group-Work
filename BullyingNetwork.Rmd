...
This third version of the network exploits `visNetwork` to build an interactive visualization of the class, reproducing the first version aforehead proposed, where:

- shapes indicate the gender (i.e. females as circles, males as squares);
- colours indicate the ethnicity, as shown in the legend;
- the size is handled on the basis of the grade of the single student.
...
Note that the following plot's nodes size indicates whether the indegree
...
Moreover, the correlation test shows a low p-values, indicating a statistically significant relationship between the number of people that bullies a kid and the grade he/she gets. 
...
However, no significant p-value was obtained from these tests. This means that gender is not a discriminant in the bullying behaviour inside the class. 
...
The significance of grade will be demonstrated in the following section through the permutation based approach. 
...
Let’s start with a census about dyads, which will return how many mutual, asymmetric and null dyads there are in the network. Given $25$ nodes, the maximal number of edges is $(25 \cdot 24)/2 = 300$, while the minimal is $0$. In this specific network, there are:
