...
These values are high above the average of `r avg_outdegree` $\pm$ `r round(sd(people$outdegree),2)` but do reach extreme cases, such as a centralized network (i.e., maximum outdegree). Therefore, a visual interpretation of the out-degree distribution in a bar plot graph may help understand the underlying reasons for low reference points.    
...
formattable(data.frame(cbind("Betweenness" = betweenness,
            ...
            list(
                'Betweenness' = color_bar(tile_colors[1], proportion),
                "Constraint.Index" = color_bar(tile_colors[3]),
                "Bonacich.Power.Centrality" = color_bar(tile_colors[2]), proportion),
            align = c("r","c","l"))
```
Comment betweenness

As defined by Borgatti et al.^[Stephen P. Borgatti, Martin G. Everett, Jeffrey C. Johnson. *Analyzing Social Networks*, 2nd Edition (2018).], Burt's constraint index is another way of measuring the extent to which ego’s alters have ties to each other by measuring the extent to which ego invests time and energy in alters who invest in each other. 
