...
For completeness, some tests are run for ethnicity too. Since there are three groups of ethnicity amongst students, ANOVA tests are used to indagate pairwise differences. As `TukeyHSD` shows, the p-value is not significant, therefore ethnicity is not discriminant for bullying or being bullied. 
```{r}
...
Given $25$ nodes, the maximal number of edges is $(25*24)/2 = 300$, while the minimal is $0$. According to the census, there are:
